...
# 과목별 기본 문항 유형(객관식/주관식) 설정 함수 (수학 16-21, 29-30 주관식)
            ...
            with st.form("grading_form"):
                ...
                for q in range(1, note_data["num_questions"] + 1):
                    ...
                    with grading_cols[col_idx]:
                        ...
                        q_type_str = note_data['q_types'].get(q, '객관식')
                        st.write(f"**{q}번 ({q_type_str})** [마킹: {u_ans_display}]")
                        
                        # 직접 입력(st.text_input) 방식
                        default_ans = "1" if q_type_str == "객관식" else ""
                        ans_val = st.text_input(f"{q}번 정답", value=default_ans, key=f"ans_input_{q}")
                        
                        default_score = str(default_scores.get(q, 2))
